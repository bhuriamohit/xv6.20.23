...
int history(char *buff, int id) {
    ...
    memmove(buff, historytable[id].buf, len);
...
arrowindex--;
   ...
   int length=historytable[arrowindex%MAX_HISTORY].len;

   for(int i=0;i<length;i++){

char curr=historytable[arrowindex%MAX_HISTORY].buf[i];
...
arrowindex++;
  ...
  if(arrowindex<historyindex){ int length=historytable[arrowindex%MAX_HISTORY].len;
...
void saveCommandInHistory(){
  ...
  for (uint i = 0; i < len; i++) { 
    ...
    historytable[historyindex%MAX_HISTORY].buf[i] =  input.buf[(input.r + i) % INPUT_BUF];

  }

  historytable[historyindex%MAX_HISTORY].len=len;
